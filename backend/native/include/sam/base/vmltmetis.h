...
#ifndef _THREAD_METIS_H_
#define _THREAD_METIS_H_
#include <cmath>
...
#define THREAD_METIS_VER_MAJOR    5
#define THREAD_METIS_VER_MINOR    1
#define THREAD_METIS_VER_SUBMINOR 0

/* The maximum length of the options[] array */
#define THREAD_METIS_NOPTIONS 40
...
namespace VKI_Metis {
    ...
    METIS_API(int)
    ThreadMETIS_NodeND(idx_t numThreads, idx_t* nvtxs, idx_t* xadj, idx_t* adjncy, idx_t* vwgt, idx_t* options, idx_t* perm,
                       idx_t* iperm);

    // METIS_API(int) METIS_Free(void *ptr);

    METIS_API(int)
    ThreadMETIS_SetDefaultOptions(idx_t* options);
...
typedef enum { METIS_IPTYPE_GROW, METIS_IPTYPE_RANDOM, METIS_IPTYPE_EDGE, METIS_IPTYPE_NODE, METIS_IPTYPE_METISRB } miptype_et;
