...
void
draw_ElemFaceContour(vis_Group* group, vis_Connect* connect, vis_State* state, vis_State* stated, vis_IsoClip* isoclip,
                     vis_Contour* contour);
void
draw_ElemCutPlane(vis_Group* group, vis_Connect* connect, vis_State* state, vis_State* stated, vis_DataInt* dataint,
...
main()
{
    ...
    vis_VisContext *vc, *vccut;
    vis_VisContext *vcvalue, *vctriad, *vclegend;
    vis_Levels *levels, *levelscut;
    ...
    vis_Value* value;
    vis_Contour* contour;
    vis_Threshold *threshold, *thresholdcut;
    vis_Triad* triad;
    vis_Legend* legend;
    vis_IsoClip* isoclip;
    vis_DataInt* dataint;
    ...
    vgl_Xfm* xfm;
    vgl_RasFont* rasfont;
    vgl_DrawFun *df, *dfcut;
    vgl_DList* dl;
    vgl_DrawFun *dfedge, *dftitle, *dftriad, *dflegend;
    vgl_DList *dledge, *dltitle, *dltriad, *dllegend;
    vgl_IActor* iactor;
    vgl_Popup* popup;
    ...
    vgl_DrawFunOpenWindow(dfdev, "Example 30bvgl");
    ...
    /* cutting plane objects */
    /* create isoclip object */
    isoclip = vis_IsoClipBegin();
    /* set cutting plane towards end of x coordinate extent */
    cutx = .25F * extent[0][0] + .75F * extent[1][0];
    vis_IsoClipSetType(isoclip, 0, ISOCLIP_DATA, ISOCLIP_HITHER, cutx);

    /* use function provided by Contour module */
    dfcut = vgl_DrawFunBegin();
    vgl_DrawFunSetObj(dfcut, contour);
    vgl_DrawFunAPI(dfcut, DRAWFUN_APIRETURN);
    vgl_DrawFunSet(dfcut, DRAWFUN_POLYGONDATA, (Vfunc*)vis_ContourPolygonData);

    /* create levels object with one level at cutting plane */
    levelscut = vis_LevelsBegin();
    vis_LevelsDef(levelscut, LEVELS_LINEAR, 1);
    vis_LevelsSetValue(levelscut, 1, cutx);

    /* vis context and set attributes */
    vccut = vis_VisContextBegin();
    vis_VisContextSetIsoValType(vccut, VIS_ISOVALSURFACE);
    vis_VisContextSetDraw(vccut, SYS_OFF);

    /* Threshold data interpolation */
    dataint = vis_DataIntBegin();

    /* create threshold object and set objects */
    thresholdcut = vis_ThresholdBegin();
    vis_ThresholdSetObject(thresholdcut, VGL_DRAWFUN, dfcut);
    vis_ThresholdSetObject(thresholdcut, VIS_VISCONTEXT, vccut);
    vis_ThresholdSetObject(thresholdcut, VIS_LEVELS, levelscut);
    vis_ThresholdSetObject(thresholdcut, VIS_DATAINT, dataint);
    ...
    vsy_PropSetInsertp(prop, "thresholdcut", (Vobject*)thresholdcut);
    vsy_PropSetInsertp(prop, "isoclip", (Vobject*)isoclip);
    vsy_PropSetInsertp(prop, "dataint", (Vobject*)dataint);
    ...
    vgl_PopupAddItem(popup, 3, (Vchar*)"Element CutPlane", (void (*)(vgl_Popup*, Vobject*))popup_callback, prop);
    vgl_PopupAddItem(popup, 4, (Vchar*)"Element Isosurface", (void (*)(vgl_Popup*, Vobject*))popup_callback, prop);
    ...
    vgl_PopupAddItem(popup, 7, (Vchar*)"Node Label", (void (*)(vgl_Popup*, Vobject*))popup_callback, prop);
...
popup_callback(vgl_Popup* popup, vsy_PropSet* prop)
{
    ...
    vsy_PropSetLookupObject(prop, "thresholdcut", (Vobject**)&thresholdcut);
    vsy_PropSetLookupObject(prop, "isoclip", (Vobject**)&isoclip);
    vsy_PropSetLookupObject(prop, "dataint", (Vobject**)&dataint);
    ...
    if (ind == 0) {
        ...
        draw_ElemFaceContour(groupface, connect, state, NULL, NULL, contour);
    }
    else if (ind == 3) {
        draw_title(dftitle, (Vchar*)"Element CutPlane");
        vgl_DListSetVisibility(dllegend, VGL_ON);
        vis_StateSetDerive(state, VIS_VECTOR_MAG);
        vis_VisContextSetIsoValType(vc, VIS_ISOVALFRINGE);
        draw_ElemFaceContour(groupface, connect, state, NULL, isoclip, contour);
        draw_ElemCutPlane(NULL, connect, state, NULL, dataint, thresholdcut);
    }
    else if (ind == 4) {
                     ...
                     vis_Contour* contour)
{
    ...
    /* initialize isoclip */
    if (isoclip != NULL) {
        vis_IsoClipSetDataPtr(isoclip, 0, cp);
        vis_ContourSetObject(contour, VIS_ISOCLIP, isoclip);
    }
    ...
    /* finalize isoclip */
    if (isoclip != NULL) {
        vis_ContourSetObject(contour, VIS_ISOCLIP, NULL);
    }
}

/*----------------------------------------------------------------------
                      Draw Element CutPlane
----------------------------------------------------------------------*/
void
draw_ElemCutPlane(vis_Group* group, vis_Connect* connect, vis_State* state, vis_State* stated, vis_DataInt* dataint,
                  vis_Threshold* threshold)
{
    ...
    /* interpolate scalar value in s */
    vis_DataIntSetDataPtr(dataint, 0, 1, s);
    ...
    for (n = 1; n <= numel; n++) {
        ...
        if (shape == SYS_SHAPETET || shape == SYS_SHAPEWED || shape == SYS_SHAPEHEX) {
            ...
            vis_ThresholdCurv(threshold, cp, x, VIS_NODATA, NULL);
        }
    }
}
