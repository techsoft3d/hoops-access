...
static Vchar* vdmname[] = {(Vchar*)"VDM_NATIVE",
                           (Vchar*)"VDM_LSTC_STATE",
                           (Vchar*)"VDM_ABAQUS_FIL",
                           (Vchar*)"VDM_ABAQUS_ODB",
                           (Vchar*)"VDM_NASTRAN_OUTPUT2",
                           (Vchar*)"VDM_SDRC_UNIVERSAL",
                           (Vchar*)"VDM_PAM_DAISY",
                           (Vchar*)"VDM_ANSYS_RESULT",
                           (Vchar*)"VDM_MECHANICA_STUDY",
                           (Vchar*)"VDM_PATRAN_NEUTRAL",
                           NULL};

static Vint vdmtype[] = {VDM_NATIVE,         VDM_LSTC_STATE, VDM_ABAQUS_FIL,   VDM_ABAQUS_ODB,      VDM_NASTRAN_OUTPUT2,
                         VDM_SDRC_UNIVERSAL, VDM_PAM_DAISY,  VDM_ANSYS_RESULT, VDM_MECHANICA_STUDY, VDM_PATRAN_NEUTRAL};
...
main(int argc, char** argv)
{
    ...
    if (vdm_DataFunError(datafun)) {
        if (filetype == VDM_ABAQUS_ODB) {
            vdm_ABALibGetInteger(abalib, ABALIB_UPGRADEODB, &upgrade);
            if (upgrade) {
                sprintf(gradefile, "new_%s", inputfile);
                vdm_ABALibUpgradeOdb(abalib, inputfile, gradefile);
                vdm_ABALibEnd(abalib);
                abalib = vdm_ABALibBegin();
                vdm_ABALibDataFun(abalib, datafun);
                vdm_DataFunOpen(datafun, 0, gradefile, filetype);
                if (vdm_DataFunError(datafun)) {
                    fprintf(stderr, "Upgraded, but error still detected\n");
                    exit(0);
                }
            }
            else {
                fprintf(stderr, "Error: No upgrade available\n");
                exit(0);
            }
        }
        else {
